...
class Trainer:
    ...
    def train(self, round=1):
            ...
            for i, data in enumerate(self.trainset_loader, 0):
                ...
                if i % (len(self.trainset_loader) // 10) == (len(self.trainset_loader) // 10) - 1:
                    ...
                    if self.config['view_adaptated_image']:
                        Imshow.imshow(inputs[0], self.net.forward_adaptation(inputs[0]))
                    if self.config['save_adaptated_image']:
        ...
        np.savetxt(f'./log/{model_name}/train_acc_{round}.txt', train_acc_list)
        np.savetxt(f'./log/{model_name}/test_acc_{round}.txt', test_acc_list)
