...
from datasets.dataset_loader import MyDataSet
...
if config['view_adaptated_image']:
    ...
    for i in range(0, 32):
        Imshow.imshow(images[i], net.forward_adaptation(images[i]), f'./log/{ConfigLoader.generate_model_name(config)}/adapted_images/image{i}.png')
...
for round in range(1, config['training_round'] + 1):
    ...
    AccuracyPlotter(config, ConfigLoader.generate_model_name(config)).plot(show=False, round=round)
